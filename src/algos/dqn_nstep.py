...
def run_dqn(args, use_tensorboard=False, use_wandb=False):
    ...
    rb = ReplayBuffer(args.buffer_size, n_step=args.n_step, gamma=args.gamma)
        ...
        if global_step > args.learning_starts:
            if global_step % args.train_frequency == 0:
                ...
                with torch.no_grad():
                    ...
                    td_target = data.reward.flatten() + (args.gamma ** args.n_step) * target_max * (1 - data.done.flatten())
                old_val = q_network(data.state, data.action_mask).gather(1, data.action).squeeze(1)  # (B,)
                td_target = td_target.view(-1)  # (B,)
