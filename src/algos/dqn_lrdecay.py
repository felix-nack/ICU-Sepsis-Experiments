...
def create_lr_scheduler(optimizer, args, total_updates: int):
    ...
    if decay == "linear":
        ...
        def lr_lambda(update_idx: int):
            ...
            # decay phase: rescale remaining part to [0,1]
            frac2 = (frac - warm_frac) / (1.0 - warm_frac)
            return (1.0 - frac2) * 1.0 + frac2 * end_factor
...
# args = parse_args()
def run_dqn_lrdecay(args, use_tensorboard=False, use_wandb=False):
    ...
    while episode_number < args.max_episodes:
        ...
        if terminated:
            ...
            if 'final_info' in infos:
                ...
                total_reward = infos['episode']['r'][0]
                episode_length = infos['episode']['l'][0]
            else:
                # Fallback: calculate from accumulated rewards
                total_reward = rewards[0] if hasattr(rewards, '_len_') else rewards
                episode_length = global_step - start_index
...
if __name__ == '__main__':
    ...
    run_dqn_lrdecay(args, use_tensorboard=True, use_wandb=args.track)