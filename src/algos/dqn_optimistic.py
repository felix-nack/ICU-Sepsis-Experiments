...
def run_dqn_optimistic(args, use_tensorboard=False, use_wandb=False):
    ...
    target_network = QNetworkOptimistic(envs, bias_const=bias_const).to(device)
    target_network.load_state_dict(q_network.state_dict())
    n_states = envs.single_observation_space.n
    n_actions = envs.single_action_space.n
    print(f"Number of States: {n_states}, Number of actions: {n_actions}")
    print(f"Using Optimistic Q-Initialization with bias constant: {bias_const}")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--bias-const", type=float, default=1.0,
        help="constant value for optimistic bias initialization (higher = more optimistic)")
